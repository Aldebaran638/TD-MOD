...
try {
    ...
    Assert-True ([string]$output -match 'block' -and [string]$output -match 'Continue with Step \d+\.\d+' -and [string]$output -notmatch 'Continue with Step 0\.1|Continue with Step 0\.3|Continue with Step 0\.4|Continue with Step 1\.5|Continue with Step 1\.7') 'current source skips the exceptional Step 0.1, Step 0.3, Step 0.4, Step 1.5, and Step 1.7 and requests the next unresolved task'
