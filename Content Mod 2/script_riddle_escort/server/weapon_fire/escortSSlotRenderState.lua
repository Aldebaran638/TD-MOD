...
server.escortSSlotRenderState = server.escortSSlotRenderState or {
    seq = 0,
    shotId = 0,
}

local function _escortSSlotRenderStateVec3ToArray(v, defaultX, defaultY, defaultZ)
    local t = v or {}
    local x = t.x
...
function server.escortSSlotRenderStateInit()
    server.escortSSlotRenderState = {
        seq = 0,
        shotId = 0,
    }
    return server.escortSSlotRenderState
end

function server.escortSSlotRenderPushEvent(shipBodyId, payload)
    if shipBodyId == nil or shipBodyId == 0 then
        return false
    end

    local state = server.escortSSlotRenderState
    if state == nil then
        state = server.escortSSlotRenderStateInit()
    end
    ...
    local firePoint = _escortSSlotRenderStateVec3ToArray(p.firePoint, 0.0, 0.0, 0.0)
    local hitPoint = _escortSSlotRenderStateVec3ToArray(p.hitPoint, 0.0, 0.0, 0.0)
    local normal = _escortSSlotRenderStateVec3ToArray(p.normal, 0.0, 1.0, 0.0)
    ...
    ClientCall(
        0,
        "client.receiveEscortSSlotRenderEvent",
        shipBodyId,
        state.seq,
        state.shotId or 0,
