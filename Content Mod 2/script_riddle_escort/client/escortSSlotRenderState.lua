...
client.escortSSlotRenderStateByShip = client.escortSSlotRenderStateByShip or {}

local function _escortSSlotRenderEnsureClientState(shipBodyId)
    local body = math.floor(shipBodyId or 0)
    if body <= 0 then
        return nil
    end

    local states = client.escortSSlotRenderStateByShip
...
function client.receiveEscortSSlotRenderEvent(
    shipBodyId,
    seq,
    shotId,
    eventType,
    ...
    local state = _escortSSlotRenderEnsureClientState(shipBodyId)
    if state == nil then
        return
    end
...
function client.escortSSlotRenderGetEvent(shipBodyId)
    local body = math.floor(shipBodyId or 0)
    if body <= 0 then
        return nil
    end
    return client.escortSSlotRenderStateByShip[body]
end
