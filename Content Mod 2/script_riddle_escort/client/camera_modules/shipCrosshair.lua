...
local function _resolveCrosshairRange()
    ...
    local maxRange = 0
    local weaponNames = { "gammaLaser", "naniteFlakBattery", "devastatorTorpedoes" }
    for i = 1, #weaponNames do
        local def = defs[weaponNames[i]] or {}
        maxRange = math.max(maxRange, tonumber(def.maxRange) or tonumber(def.range) or 0)
    end
    if maxRange <= 0 then
