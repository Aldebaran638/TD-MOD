...
local function _sSlotApplyShipDamage(hitBody, missile)
    ...
    local targetShipData = (shipData and shipData[targetShipType]) or (shipData and shipData[server.defaultShipType or "titan"]) or {}
...
function server.sSlotStateInit(shipType)
    ...
    local slotDefs = shipDef.mSlots or shipDef.sSlots or {}
    for i = 1, #slotDefs do
        state.launchers[i] = {
...
function server.sSlotControlTick(dt)
    ...
    if server.shipRuntimeGetCurrentMainWeapon ~= nil and server.shipRuntimeGetCurrentMainWeapon(shipBody) ~= "mSlot" then
        return
    end
