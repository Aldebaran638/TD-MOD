...
function client.shipCameraApplyProfile()
    local profile = client.shipContextGetDefinition().cameraProfile or {}
    local camera = client.shipCamera
    camera.r = tonumber(profile.distance) or camera.r
    camera.rMin = tonumber(profile.distanceMin) or camera.rMin
    camera.rMax = tonumber(profile.distanceMax) or camera.rMax
    camera.angleLimitPitch = tonumber(profile.pitchLimit)
        or camera.angleLimitPitch
    camera.angleLimitYaw1 = tonumber(profile.rearYawMin)
        or camera.angleLimitYaw1
    camera.angleLimitYaw2 = tonumber(profile.rearYawMax)
        or camera.angleLimitYaw2
    camera.mouseSensitivity = tonumber(profile.mouseSensitivity)
        or camera.mouseSensitivity
    camera.glideStrength = tonumber(profile.glideStrength)
        or camera.glideStrength
    camera.zoomSpeed = tonumber(profile.zoomSpeed) or camera.zoomSpeed
    camera.switchDuration = tonumber(profile.switchDuration)
        or camera.switchDuration
    camera.frontOffset = profile.frontOffset or camera.frontOffset
    camera.frontAimPitchLimit = tonumber(profile.frontPitchLimit)
        or camera.frontAimPitchLimit
    camera.frontAimYaw1 = tonumber(profile.frontYawMin) or camera.frontAimYaw1
    camera.frontAimYaw2 = tonumber(profile.frontYawMax) or camera.frontAimYaw2
    camera.rearDefaultPitch = tonumber(profile.rearDefaultPitch)
        or camera.rearDefaultPitch
    camera.freelookTurnYawError = tonumber(profile.freelookTurnYawError)
        or camera.freelookTurnYawError
    camera.freelookTurnPitchError = tonumber(profile.freelookTurnPitchError)
        or camera.freelookTurnPitchError
