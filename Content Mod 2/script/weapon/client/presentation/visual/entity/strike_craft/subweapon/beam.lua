...
function client.spawnHSlotBeamFx(sx, sy, sz, ex, ey, ez, didHitShield, life, width, impactLayer)
    ...
    local profile = client.gammaLaserFxProfile("strikeGamma")
    ...
    client.spawnGammaLaserMuzzleFx("strikeGamma", startPos, VecSub(endPos, startPos))
    if math.floor(didHitShield or 0) ~= 0 or tostring(impactLayer or "none") ~= "none" then
        client.spawnGammaLaserImpactFx("strikeGamma", endPos, VecSub(startPos, endPos), impactLayer or (math.floor(didHitShield or 0) ~= 0 and "shield" or "body"))
    end
    local _ = width
end
