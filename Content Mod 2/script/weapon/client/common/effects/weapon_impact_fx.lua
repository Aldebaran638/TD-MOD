...
function client.spawnWeaponImpactFx(weaponType, position, normal, impactLayer)
    ...
    if profile ~= "focusedArcImpact" and profile ~= "disruptorImplosion" then return false end
    ...
    local effect = { profile = profile, position = position, normal = _normal(normal), age = 0, life = profile == "focusedArcImpact" and 0.18 or 0.20, layer = impactLayer }
    table.insert(active, effect)
    local count = profile == "focusedArcImpact" and 12 or 18
...
function client.weaponImpactFxRender()
    for _, effect in ipairs(client.weaponImpactFxState.active) do
        ...
        local color = effect.profile == "focusedArcImpact" and { 0.78, 0.25, 1.0 } or { 0.16, 1.0, 0.35 }
        local size = effect.profile == "focusedArcImpact" and (1.4 + t * 2.2) or (2.6 * (1 - t) + 0.08)
        ...
        if t < .18 then client.weaponFxPointLight(effect.position, color[1], color[2], color[3], 10 * alpha) end
    end
end
