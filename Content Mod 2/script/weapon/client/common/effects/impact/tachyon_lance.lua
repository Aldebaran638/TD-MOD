-- Tachyon lance impact effect.
---@diagnostic disable: undefined-global
---@diagnostic disable: duplicate-set-field

client = client or {}

client.tachyonImpactFxState = client.tachyonImpactFxState or {
    lastRenderSeqByShip = {},
    lastShotIdByShip = {},
    activeTachyonImpacts = {},
}

client.tachyonImpactFxConfig = client.tachyonImpactFxConfig or {
...
local function _resolveLayerColors(impactLayer, weaponType)
    ...
    if tostring(definition.fxPalette or "") ~= "" then
        local palette = client.weaponFxGetPalette(definition.fxPalette)
        local color = palette.impactColor
        return color[1], color[2], color[3], color[1] * 0.55, color[2] * 0.08, color[3] * 0.05
    end
...
local function _spawnTachyonImpact(pos, normal, backDirection, impactLayer, weaponType)
    ...
    local impacts = client.tachyonImpactFxState.activeTachyonImpacts or {}
    client.tachyonImpactFxState.activeTachyonImpacts = impacts
    impacts[#impacts + 1] = {
...
function client.tachyonImpactFxTick(dt)
    local state = client.tachyonImpactFxState
    local config = client.tachyonImpactFxConfig
