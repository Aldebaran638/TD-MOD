...
local function _tachyonBeamDefinition(weaponType)
    return (weaponData or {})[tostring(weaponType or "")] or {}
end
...
local function _tachyonBeamPalette(weaponType)
    ...
    if tostring(definition.fxPalette or "") ~= "" then
        return {
            { thickness = 72.0, color = palette.beamOuterColor, intensityScale = 0.70, alpha = 0.75 },
            { thickness = 42.0, color = palette.beamOuterColor, intensityScale = 0.80, alpha = 0.85 },
            { thickness = 24.0, color = palette.beamCoreColor, intensityScale = 0.85, alpha = 0.92 },
        }, palette.beamCoreColor
    end
    return nil, nil
end
        ...
        if render ~= nil then
            ...
            if seq ~= state.lastRenderSeq then
                ...
                if render.eventType == "launch_start"
                    ...
                    state.launchAge = 0.0
                    state.weaponType = tostring(render.weaponType or "")
                end
            end
        end
    end
