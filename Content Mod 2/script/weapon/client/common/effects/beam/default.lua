...
for profileId, profile in pairs(_profiles) do
    client.weaponFxRegister("beam", profileId, { renderer = "generic", config = profile })
end
...
function client.spawnGenericRaycastWeaponFx(
    ...
    local registered = client.weaponFxResolve("beam", profileId) or {}
    local profile = registered.config or _profiles[profileId] or _profiles.energyBeam
    ...
    local life = profileId == "gammaBeam" and client.gammaLaserFxProfile(gammaProfile).life or profile.life
    ...
    if muzzleFxProfile == "gammaLarge" or muzzleFxProfile == "gammaMedium" then
        client.spawnGammaLaserMuzzleFx(muzzleFxProfile, startPos, direction)
    ...
    if math.floor(didHit or 0) ~= 0 and profileId ~= "perditionBeam" then
        if impactFxProfile == "gammaLarge" or impactFxProfile == "gammaMedium" then
            client.spawnGammaLaserImpactFx(impactFxProfile, endPos, hitNormal, impactLayer)
    ...
    for i = 1, #(state.beams or {}) do
        ...
        if length > 0.001 then
            ...
            if beam.profile == "gammaBeam" then
                client.gammaLaserDrawBeam(beam.startPos, beam.endPos, client.gammaLaserFxProfile(beam.gammaProfile), (beam.maxLife - beam.life), beam.maxLife)
