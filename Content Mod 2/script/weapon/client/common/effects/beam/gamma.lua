...
for profileId, profile in pairs(GammaProfiles) do
    client.weaponFxRegister("beam", profileId, { renderer = "gamma", config = profile })
    client.weaponFxRegister("muzzle", profileId, { renderer = "gamma", config = profile })
    client.weaponFxRegister("impact", profileId, { renderer = "gamma", config = profile })
end
...
function client.gammaLaserFxProfile(profileId)
    return GammaProfiles[tostring(profileId or "")] or GammaProfiles.gammaMedium
end

function client.spawnGammaLaserMuzzleFx(profileId, position, direction)
    local profile = client.gammaLaserFxProfile(profileId)
    local state = client.gammaLaserFxState
...
function client.spawnGammaLaserImpactFx(profileId, position, normal, impactLayer)
    local profile = client.gammaLaserFxProfile(profileId)
    local state = client.gammaLaserFxState
