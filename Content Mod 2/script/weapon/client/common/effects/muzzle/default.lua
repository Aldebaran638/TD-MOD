...
for profileId, profile in pairs(Profiles) do
    client.weaponFxRegister("muzzle", profileId, { renderer = "generic", config = profile })
end
...
function client.spawnWeaponMuzzleFx(weaponType, px, py, pz, dx, dy, dz)
    ...
    local profileId = tostring(definition.muzzleFxProfile or "")
    local registered = client.weaponFxResolve("muzzle", profileId) or {}
    local profile = registered.config or Profiles[profileId]
