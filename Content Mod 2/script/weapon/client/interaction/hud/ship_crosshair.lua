...
local function _resolveCrosshairRange()
    ...
    local maxRange = 0.0
    for _, definition in pairs(defs) do
        local candidate = tonumber(definition.maxRange or definition.range) or 0.0
        if candidate > maxRange then maxRange = candidate end
    end
    if maxRange <= 0 then
