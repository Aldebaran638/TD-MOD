...
local function _resolveWeaponSettings(weaponType)
    ...
    local resolvedWeaponType = tostring(weaponType or "")
    local settings = defs[resolvedWeaponType] or {}
...
function server.xSlot_applyHitResult(
    ...
    local resolvedWeapon = (weaponData or {})[tostring(weaponType or "")] or {}
    ...
    if isHitStellarisBody then
        ...
        local targetWeaponData = (weaponData and weaponData[weaponType]) or {}
...
function server.xSlot_broadcastWeaponIdle(shipBodyId, slotIndex, weaponType, firePointWorld)
    server.tachyonMuzzleLightStop(weaponType)
    server.xSlotRenderPushEvent(shipBodyId, {
        eventType = "idle",
        slotIndex = slotIndex,
        weaponType = weaponType,
