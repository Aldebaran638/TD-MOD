...
if ($lua -notmatch 'promotionAllowed = true' -or $lua -notmatch 'mode = "promoted"') { throw "promoted candidate fixture is missing" }
if (@($manifest.runtimeCatalogs).Count -ne 2) { throw "runtime projection manifest entries are missing" }
Write-Host "[PASS] generated header/hash/manifest/entry closure and explicit promoted ownership are present"
