...
local function _runtimeResolveShipDefinition(shipType, defaultShipType)
    local requestedShipType = shipType or defaultShipType or "titan"
    local defs = shipTypeRegistryData or {}
    local definition = defs[requestedShipType] or defs[defaultShipType] or defs.titan or {}
...
local function _runtimeNormalizeMode(mode)
    ...
    if mode == "mSlot" or mode == "sSlot" then
        return "mSlot"
    end
    return "tSlot"
end
