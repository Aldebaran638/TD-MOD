...
function server.shipRequestTWeaponHold(playerId, shipBodyId, request)
    if server.shipBody == nil or server.shipBody == 0 or server.shipBody ~= shipBodyId then
        return false
    end
    if not _canAcceptShipRequest(playerId, shipBodyId) then
    ...
    local current = server.shipRuntimeGetCurrentMainWeapon ~= nil and server.shipRuntimeGetCurrentMainWeapon(shipBodyId) or "tSlot"
    if current ~= "tSlot" then
        return false
    end
...
function server.shipRequestTWeaponRelease(playerId, shipBodyId)
    if server.shipBody == nil or server.shipBody == 0 or server.shipBody ~= shipBodyId then
        return false
    end
    if not _canAcceptShipRequest(playerId, shipBodyId) then
